...
import { Cafe, STRAPI_URL } from "@/app/api/interface";
...
export function CafeCard({ cafe }: CafeCardProps) {
    ...
    return (
        ...
        >
            ...
            />
            <div className="p-4">
                <h3 className="text-lg font-bold mb-2 text-shadow-white">{cafe.name}</h3>
                    ...
                    {cafe.brew_methods?.map((method) => (
                        <div key={method.id} title={method.name}>
                            {method.icon?.url ? (
                                ...
                                <span className="text-xs border p-1 rounded">{method.name}</span>
                            )}
                        </div>
                    ))}
                </div>
